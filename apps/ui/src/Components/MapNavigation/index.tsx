...
import { Map as MaplibreMap, NavigationControl, Marker, Popup, useControl } from 'react-map-gl/maplibre';
...
const MultiCarMap: React.FC<MultiCarMapProps> = ({ cars, mapStyle }) => {
    ...
    return (
        <MaplibreMap initialViewState={INITIAL_VIEW_STATE} mapStyle={mapStyles[mapStyle]}>
            ...
            {cars.map((car) => (
                <Marker key={car.id} longitude={car.startPoint[0]} latitude={car.startPoint[1]} onClick={() => handleCarSelect(car)}>
                    <div style={{ cursor: 'pointer', textAlign: 'center' }}>
                        <img src={CarIcon} alt="Car" style={{ width: '24px', height: '24px' }} />
                    </div>
                </Marker>
            ))}
            {selectedCar && selectedCar.destinations.map((destination, index) => (
